# Linear and Logistical Regressions, and Classification
...
![equation.png](Linear%20and%20Logistical%20Regressions,%20and%20Classificat/equation.png)
...
![model-prediction.png](Linear%20and%20Logistical%20Regressions,%20and%20Classificat/model-prediction.png)
...
**Figure 6**. A car's displacement in cubic centimetres and its miles per gallon rating. As a car's engine gets bigger, its miles per gallon rating generally decreases.
...
![Figure 8. Loss lines connect the data points to the
model.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/loss-lines.png)
...
| --- | --- | --- |
| [**$L_1$ loss**](https://developers.google.com/machine-learning/glossary#l1-loss) | The sum of the absolute values of the difference between the predicted values and the actual values. | $∑ | \ actual \ value−predicted \ value \|$ |
| [**Mean absolute error (MAE)**](https://developers.google.com/machine-learning/glossary#mean-absolute-error-mae) | The average of $L_1$ losses across a set of *N* examples. | $1/N∑|actual value−predicted value|$ |
...
| [**Mean squared error (MSE)**](https://developers.google.com/machine-learning/glossary#mean-squared-error-mse) | The average of $L_2$ losses across a set of *N* examples. | $1/N∑(actual value−predicted value)^2$ |
...
Using the previous [best fit line](https://developers.google.com/machine-learning/crash-course/linear-regression#linear_regression_equation), we'll calculate L2 loss for a single example. From the best fit line, we had the following values for weight and bias:
...
| L2 loss | $(actual value−predicted value)^2
= 
(26−23.1)^2$ | 8.41 |
...
![Figure 11. Illustration of the gradient descent process.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/gradient-descent.png)
...
∂∂w1M∑i=1M(fw,b(x(i))−y(i))2

and evaluates to:

1M∑i=1M(fw,b(x(i))−y(i))∗2x(i)
...
∂∂b1M∑i=1M(fw,b(x(i))−y(i))2

and evaluates to:

1M∑i=1M(fw,b(x(i))−y(i))∗2
...
You can see that the loss gets lower with each updated weight and bias. In this example, we stopped after six iterations. In practice, a model trains until it [**converges**](https://developers.google.com/machine-learning/glossary#convergence). When a model converges, additional iterations don't reduce loss more because gradient descent has found the weights and bias that nearly minimize the loss.
...
![Figure 12. Graph of loss curve showing a steep decline and then a gentle decline.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/convergence.png)
...
![Figure 13. Loss curve and corresponding graph of the model, which tilts away from the data points.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/large-loss.png)
...
![Figure 14. Loss curve and corresponding graph of the model, which cuts through the data points but not at the optimal angle.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/med-loss.png)
...
![Figure 15. Loss curve and corresponding graph of the model, which fits the data well.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/low-loss.png)
...
![Figure 16. 3-D graph of loss surface.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/convexity.png)
...
![Figure 17. 3-D graph of loss surface, with (-5.44, 35.94, 5.54) at the bottom.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/loss-weight-bias.png)
...
![Figure 18. Convex 3-D loss surface with gradient descent points moving to the lowest point.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/loss-surface-points.png)
...
![Figure 19. Graph of pounds in 1000s vs miles per gallon, with the model fitting the data.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/graphed-model.png)
...
The learning rate determines the magnitude of the changes to make to the weights and bias during each step of the gradient descent process. The model multiplies the gradient by the learning rate to determine the model's parameters (weight and bias values) for the next iteration. In the third step of [gradient descent](https://developers.google.com/machine-learning/crash-course/linear-regression/gradient-descent), the "small amount" to move in the direction of negative slope refers to the learning rate.
...
![Figure 20. Loss curve that shows a steep slope before flattening out.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/correct-lr.png)
...
![Figure 21. Loss curve that shows an almost 45-degree slope.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/small-lr.png)
...
![Figure 22. Loss curve that shows jagged up-and-down line.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/high-lr.png)
...
![Figure 23. Loss curve that shows the loss increasing at later iterations](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/increasing-loss.png)
...
- **Stochastic gradient descent (SGD)**: Stochastic gradient descent uses only a single example (a batch size of one) per iteration. Given enough iterations, SGD works but is very noisy. "Noise" refers to variations during training that cause the loss to increase rather than decrease during an iteration. The term "stochastic" indicates that the one example comprising each batch is chosen at random.
    ...
    ![Figure 24. Steep loss curve that flattens out, but with a lot of tiny fluctuations.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/noisy-gradient.png)
...
- **Mini-batch stochastic gradient descent (mini-batch SGD)**: Mini-batch stochastic gradient descent is a compromise between full-batch and SGD. For N number of data points, the batch size can be any number greater than 1 and less than N. The model chooses the examples included in each batch at random, averages their gradients, and then updates the weights and bias once per iteration.
    ...
    ![Figure 25. Steep loss curve that begins to flatten out, with much smaller fluctuations near convergence.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/mini-batch-sgd.png)
...
![Figure 26. A full batch is the entire dataset, a mini batch is a subset of the dataset, and an epoch is a full pass through ten mini batches.](https://developers.google.com/static/machine-learning/crash-course/linear-regression/images/batch-size.png)
...
- *e* is [Euler's number](https://wikipedia.org/wiki/E_(mathematical_constant)): a mathematical constant ≈ 2.71828.
...
![Sigmoid (s-shaped) curve plotted on the Cartesian coordinate plane,
         centered at the origin.](https://developers.google.com/static/machine-learning/crash-course/logistic-regression/images/sigmoid_function_with_axes.png)
...
- *z* is the output of the linear equation, also called the [**log odds**](https://developers.google.com/machine-learning/glossary#log-odds).
- *b* is the bias.
...
In the equation z=b+w1x1+w2x2+…+wNxN, *z* is referred to as the *log-odds* because if you start with the following sigmoid function (where y is the output of a logistic regression model, representing a probability):
...
![Left: Line with the points (-7.5, –10), (-2.5, 0), and (0, 5)
         ...
         points (-10, 0.00004), (0, 0.5), and (5, 0.9933) highlighted.](https://developers.google.com/static/machine-learning/crash-course/logistic-regression/images/linear_to_logistic.png)
...
$$
Log Loss=∑_{(x,y)∈D} − ylog⁡(y′)−(1−y)log⁡(1−y′)
$$

where:

- (x,y)∈D is the dataset containing many labeled examples, which are (x,y) pairs.
- y is the label in a labeled example. Since this is logistic regression, every y value of  must either be 0 or 1.
...
In the [Logistic regression module](https://developers.google.com/machine-learning/crash-course/logistic-regression), you learned how to use the [**sigmoid function**](https://developers.google.com/machine-learning/glossary#sigmoid-function) to convert raw model output to a value between 0 and 1 to make probabilistic predictions—for example, predicting that a given email has a 75% chance of being spam. But what if your goal is not to output probability but a category—for example, predicting whether a given email is "spam" or "not spam"?

[**Classification**](https://developers.google.com/machine-learning/glossary#classification-model) is the task of predicting which of a set of [**classes**](https://developers.google.com/machine-learning/glossary#class) (categories) an example belongs to. In this module, you'll learn how to convert a logistic regression model that predicts a probability into a [**binary classification**](https://developers.google.com/machine-learning/glossary#binary-classification) model that predicts one of two classes. You'll also learn how to choose and calculate appropriate metrics to evaluate the quality of a classification model's predictions. Finally, you'll get a brief introduction to [**multi-class classification**](https://developers.google.com/machine-learning/glossary#multi-class) problems, which are discussed in more depth later in the course.
...
## Classification: Accuracy, recall, precision, and related metricsbookmark_border
...
$$
Precision = \frac{correctly\ classified\ actual\ positives}{everything\ classified as\ positives} = \frac{TP}{TP+FP}
$$
...
![Figure 1. A graph of TPR (y-axis) against FPR (x-axis) showing the
            performance of a perfect model: a line from (0,1) to (1,1).](https://developers.google.com/static/machine-learning/crash-course/images/auc_1-0.png)
...
![Figure 2. Visualization of a classifier with AUC = 1.0, where all positive examples are ranked to the right of negative examples.](https://developers.google.com/static/machine-learning/crash-course/images/no_slider_data.png)
...
![Figure 3. A graph of TPR (y-axis) against FPR (x-axis) showing the
            ...
            to (1,1).](https://developers.google.com/static/machine-learning/crash-course/images/auc_0-5.png)

***Figure 3.** ROC and AUC of completely random guesses.*

### Position recall curve
...
![Precision-recall curve example with downward convex curve from (0,1)
            to (1,0)](https://developers.google.com/static/machine-learning/crash-course/images/prauc.png)
...
![Figure 4.a. ROC/AUC graph of a model with AUC=0.65.](https://developers.google.com/static/machine-learning/crash-course/images/auc_0-65.png)

![Figure 4.b. ROC/AUC graph of a model with AUC=0.93.](https://developers.google.com/static/machine-learning/crash-course/images/auc_0-93.png)
...
The points on a ROC curve closest to (0,1) represent a range of the best-performing thresholds for the given model. As discussed in the [Thresholds](https://developers.google.com/machine-learning/crash-course/classification/thresholding), [Confusion matrix](https://developers.google.com/machine-learning/crash-course/classification/thresholding#confusion_matrix) and [Choice of metric and tradeoffs](https://developers.google.com/machine-learning/crash-course/classification/accuracy-precision-recall#choice_of_metric_and_tradeoffs) sections, the threshold you choose depends on which metric is most important to the specific use case. Consider the points A, B, and C in the following diagram, each representing a threshold:

![Figure 5. A ROC curve of AUC=0.84 showing three points on the
            convex part of the curve closest to (0,1) labeled A, B, C in order.](https://developers.google.com/static/machine-learning/crash-course/images/auc_abc.png)
...
As mentioned in the [Linear regression](https://developers.google.com/machine-learning/crash-course/linear-regression) module, calculating [**prediction bias**](https://developers.google.com/machine-learning/glossary#prediction_bias) is a quick check that can flag issues with the model or training data early on.
